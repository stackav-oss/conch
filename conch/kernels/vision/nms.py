...
def _calculate_iou_kernel(
    ...
    if row_idx >= col_block_start + cxpr_block_size or row_idx >= num_boxes:
        return
...
@triton.autotune(  # type: ignore[misc]
    configs=[
        ...
        triton.Config({"cxpr_block_size": 16384}),
    ],
    key=["num_boxes"],
)
@triton.jit  # type: ignore[misc]
def _nms_suppression_kernel(
    ...
    for current_box_idx in range(num_boxes):
        ...
        if is_kept:
            ...
            remaining_boxes = num_boxes - current_box_idx

            # Iterate blockwise through the columns
            for block_idx in range(tl.cdiv(remaining_boxes, cxpr_block_size)):
                # Only need to consider later boxes, so start from current_box_idx + 1
                ...
                iou_values = tl.load(iou_matrix_ptr + iou_row_offset + target_box_offsets, mask=target_box_mask, other=0.0)
                ...
                tl.store(keep_mask_ptr + target_box_offsets, False, mask=suppression_mask & target_box_mask)
                tl.debug_barrier()
