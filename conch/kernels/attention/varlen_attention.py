...
def _varlen_attention_compute_splits_kernel(  # noqa: PLR0913, PLR0915
    ...
    kv_index = tl.program_id(1)
    total_num_kv = tl.num_programs(1)

    kv_split_index = kv_index // tl.cdiv(total_num_kv, num_kv_splits)
    kv_head_index = kv_index % tl.cdiv(total_num_kv, num_kv_splits)

    # # What query head is this program processing?
    query_element_index = tl.program_id(2)
    ...
    # Mask to only read valid indices of the actual head size
    # head_mask = head_offsets < head_size
    query_batch_index_offset = num_previous_sequences * query_batch_stride
    ...
    needs_query_mask = needs_query_group_mask or needs_head_mask
    ...
    query = _load(query_ptr + query_batch_index_offset + query_head_index_offset + query_offsets, use_mask=needs_query_mask, mask=query_mask, other=0.0)
    ...
    for relative_cache_block_index in range(num_cache_blocks_per_split):
        ...
        if cache_block_index < current_seq_num_cache_blocks:
            ...
            needs_qk_mask = needs_query_group_mask or needs_cache_block_mask
            ...
            if needs_causal_mask:
                ...
                effective_seqlen_q_offsets = tl.full([cxpr_query_group_size_padded], query_element_index, dtype=tl.int32)
                ...
                causal_mask = effective_seqlen_q_offsets[:, None] >= effective_seqlen_k_offsets[None, :]
    ...
    output_batch_index_offset = (num_previous_sequences + query_element_index) * output_scratchpad_batch_stride
    # output_split_index_offset = kv_split_index * output_scratchpad_cache_block_stride
    ...
    _store(
        ...
        output_scratchpad_ptr + output_offsets,
        output,
    ...
    #     query_head_index
    # )
    lse_scratch_batch_index_offset = (num_previous_sequences + query_element_index) * lse_scratchpad_batch_stride
    # lse_scratch_split_index_offset = kv_split_index * lse_scratchpad_cache_block_stride
    ...
    _store(
        ...
        use_mask=needs_query_group_mask,
...
def _varlen_attention_reduce_splits_kernel(  # noqa: PLR0913
    ...
    batch_index = tl.program_id(0)
    # What 
    query_element_index = tl.program_id(1)
    # What head
    query_head_index = tl.program_id(2)
    ...
    for kv_split_index in range(num_kv_splits_this_seq):
        ...
        if ((effective_seqlen_k <= query_element_index) or not cxpr_is_causal) or is_pure_decode:
            ...
            #     head_offsets[None, :]
            # )
            output_scratchpad_batch_index_offset = (num_previous_sequences + query_element_index) * output_scratchpad_batch_stride
            # output_scratchpad_split_index_offset = kv_split_index * output_scratchpad_split_stride
            ...
            lse_scratchpad_offsets = (
                (num_previous_sequences + query_element_index) * lse_scratchpad_batch_stride +
                # query_split_offsets * lse_scratchpad_batch_stride +
                kv_split_index * lse_scratchpad_kv_split_stride +
                query_head_index
            )
    ...
    output_offsets = (
        (num_previous_sequences + query_element_index) * output_batch_stride +
...
def varlen_attention_launcher(  # noqa: PLR0913
    ...
    stage1_grid = (batch_size, num_kv_splits * num_kv_heads, max_seqlen_q)
    ...
    stage2_grid = (batch_size, max_seqlen_q, num_query_heads)
