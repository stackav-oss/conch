...
def _varlen_attention_compute_splits_kernel(  # noqa: PLR0913, PLR0915
    ...
    other_sequences_offset = tl.load(cu_seqlens_k_ptr + batch_index)
    ...
    query = tl.load(
        ...
        query_ptr + other_sequences_offset + query_offsets, mask=query_mask, other=0.0
    )
...
def _varlen_attention_reduce_splits_kernel(  # noqa: PLR0913
    ...
    for split_index in range(num_kv_splits_this_seq):
...
def varlen_attention_launcher(  # noqa: PLR0913
    ...
    Args:
        out: Tensor to write the output of the attention calculation, shape: (total_num_q, num_heads, head_size).
