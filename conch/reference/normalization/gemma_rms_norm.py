...
def _gemma_rms_norm_vllm_ref(
    ...
    from vllm.model_executor.layers.layernorm import GemmaRMSNorm  # type: ignore[import-not-found]

    layer = GemmaRMSNorm(hidden_size=weight.size(0), eps=variance_epsilon)
    layer.weight = torch.nn.Parameter(weight)

    return layer.forward_cuda(x, residual)  # type: ignore[no-any-return]
